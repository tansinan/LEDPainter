...
uint16_t Adafruit_NeoPixel::getWidth() const {
  return 5;
}
uint16_t Adafruit_NeoPixel::getHeight() const {
  return 5;
}
void Adafruit_NeoPixel::setPixel(uint16_t x, uint16_t y, uint8_t r, uint8_t g, uint8_t b) {
  x = getWidth() - 1 - x;
  if(x % 2 == 0) {
    setPixelColor(x * getHeight() + y, r, g, b);
  }
  else {
    setPixelColor((x + 1) * getHeight() - 1 - y, r, g, b);
  }
}
